...
#include "imagelib/horn.h"
#include "imagelib/image.h"
#include "matrix/matrix.h"
#include "imagelib/qsort.h"
...
double BundlerApp::RunSFMGRID(int num_pts, int num_grids, int *grid_rows, int *grid_cols, int num_cameras, int start_camera,
							  bool fix_points, camera_params_t *init_camera_params, v3_t *init_pts, 
							  int *added_order, v3_t *colors,
...
#define MIN_POINTS 10
	...
	int total_no_grid_pts = 0;
	for(int g=0; g<num_grids; g++)
	{
		total_no_grid_pts += grid_rows[g]*grid_cols[g];
	}
	int no_other_pts = num_pts - total_no_grid_pts;
	int *remap = new int [num_pts];
	
    v3_t *nz_pts = new v3_t[no_other_pts];
	v3_t *grid_pt_tmp = new v3_t[num_grids];;
	v3_t *grid_hor_trans_tmp = new v3_t[num_grids];
	v3_t *grid_ver_trans_tmp = new v3_t[num_grids];
	
	for(int g=0; g<num_grids; g++)
	{
		printf("grid %d\n", g);
		grid_pt_tmp[g] = grid_pt[g];
		grid_hor_trans_tmp[g] = grid_hor_trans[g];
		grid_ver_trans_tmp[g] = grid_ver_trans[g];
		printf("pt:%f %f %f\n", Vx(grid_pt[g]), Vy(grid_pt[g]), Vz(grid_pt[g]));
		printf("hor:%f %f %f\n", Vx(grid_hor_trans[g]), Vy(grid_hor_trans[g]), Vz(grid_hor_trans[g]));
		printf("ver:%f %f %f\n", Vx(grid_ver_trans[g]), Vy(grid_ver_trans[g]), Vz(grid_ver_trans[g]));
	}
	/*grid_pt_tmp[0].p[0] = -1.598322;
	grid_pt_tmp[0].p[1] = 1.721928;
	grid_pt_tmp[0].p[2] = -16.909246;
	grid_hor_trans_tmp[0].p[0] = 0.491288;
	grid_hor_trans_tmp[0].p[1] = -0.169100;
	grid_hor_trans_tmp[0].p[2] = -0.504868;
	grid_ver_trans_tmp[0].p[0] = 0.010228;
	grid_ver_trans_tmp[0].p[1] = -0.433787;
	grid_ver_trans_tmp[0].p[2] = 0.128970;*/
	...
	for (int i = 0; i < num_cameras; i++) 
	{
			 ...
			 iter++) 
		{
			...
			if (key.m_extra >= 0) 
			{
				...
				if(pt_idx >= total_no_grid_pts)
				{
					b[0] = Vx(init_pts[pt_idx-total_no_grid_pts]);
					b[1] = Vy(init_pts[pt_idx-total_no_grid_pts]);
					b[2] = Vz(init_pts[pt_idx-total_no_grid_pts]);
				}
				else
				{
					int prevPts = 0;
					for(int g=0; g<num_grids; g++)
					{
						if(pt_idx < prevPts + grid_rows[g]*grid_cols[g])
						{
							int r = (pt_idx-prevPts)/grid_cols[g];
							int c = (pt_idx-prevPts)%grid_cols[g];
							b[0] = Vx(grid_pt_tmp[g])+Vx(grid_hor_trans_tmp[g])*c + Vx(grid_ver_trans_tmp[g])*r;
							b[1] = Vy(grid_pt_tmp[g])+Vy(grid_hor_trans_tmp[g])*c + Vy(grid_ver_trans_tmp[g])*r;
							b[2] = Vz(grid_pt_tmp[g])+Vz(grid_hor_trans_tmp[g])*c + Vz(grid_ver_trans_tmp[g])*r;
							//printf("gridPt at (%d, %d):%f %f %f\n", r, c, b[0], b[1], b[2]);
							break;
						}
						prevPts += grid_rows[g]*grid_cols[g];
					}
    ...
    do {
		...
		//place non-grid points
		for(int i=0; i<no_other_pts; i++)
		{
			//printf("non-grid pt:%d\n", i);
			int num_views = (int) pt_views[i+total_no_grid_pts].size();
			if(num_views > 0)
			{
				for (int j = 0; j < num_views; j++) 
				{
					int c = pt_views[i+total_no_grid_pts][j].first;
					int v = added_order[c];
					int k = pt_views[i+total_no_grid_pts][j].second;
					...
					//printf("in view %d:%f %f\n", v, GetKey(v,k).m_x, GetKey(v,k).m_y);
					arr_idx++;
				}
		...
		for(int g=0; g<num_grids; g++)
		{
			for(int i=0; i<grid_rows[g]; i++)
			{
				...
				{
					printf("grid pt:%d\n", i*grid_cols[g]+j);
					int num_views = (int) pt_views[prevPts + i*grid_cols[g]+j].size();
					for (int k = 0; k < num_views; k++)
					{
						int c = pt_views[prevPts+i*grid_cols[g]+j][k].first;
						int v = added_order[c];
						int m = pt_views[prevPts+i*grid_cols[g]+j][k].second;
					
						vmask[(nz_count + prevPts + i*grid_cols[g]+j) * num_cameras + c] = 1;
					
						projections[2 * arr_idx + 0] = GetKey(v,m).m_x;
						projections[2 * arr_idx + 1] = GetKey(v,m).m_y;
					
						printf("in view:%d:%f %f\n", v, GetKey(v,m).m_x, GetKey(v,m).m_y);
						arr_idx++;
					}
				}
			}
		...
		//for(int o=0; o<nz_count; o++)
		//{
		//	printf("point %d:%f %f %f\n", o, Vx(nz_pts[o]), Vy(nz_pts[o]), Vz(nz_pts[o]));
		//}
		
        //run_sfm_grid_opt(nz_count, num_grids, grid_rows, grid_cols, num_cameras, start_camera, vmask, projections,
		//		fixed_focal ? 0 : 1, 0,
		...
		//		fix_points ? 1 : 0, m_optimize_for_fisheye, eps2, 0);
        ...
        for (int i = 0; i < num_cameras; i++) {
            ...
            for (iter = m_image_data[added_order[i]].m_keys.begin();
				...
				if (key.m_extra >= 0) {
					...
					if(pt_idx >= total_no_grid_pts)
					{
						b[0] = Vx(nz_pts[remap[pt_idx-total_no_grid_pts]]);
						b[1] = Vy(nz_pts[remap[pt_idx-total_no_grid_pts]]);
						b[2] = Vz(nz_pts[remap[pt_idx-total_no_grid_pts]]);
					}
					else
					{
						int prevPts = 0;
...
#define NUM_ERROR_BINS 10
        ...
        if (remove_outliers) {
			for (int i = 0; i < (int) outliers.size(); i++) {
                ...
                for (int j = 0; j < num_views; j++) {
                    ...
                    if(idx > total_no_grid_pts)
						vmask[(idx-total_no_grid_pts) * num_cameras + v] = 0;
					else
						vmask[(nz_count+idx) * num_cameras + v] = 0;
		...
		for(int g=0; g<num_grids; g++)
		{
			grid_pt[g] = grid_pt_tmp[g];
			grid_hor_trans[g] = grid_hor_trans_tmp[g];
			grid_ver_trans[g] = grid_ver_trans_tmp[g];
		}
		
		//for(int o=0; o<nz_count; o++)
		//{
		//	printf("point %d:%f %f %f\n", o, Vx(init_pts[o]), Vy(init_pts[o]), Vz(init_pts[o]));
		//}
...
double BundlerApp::OptimizeForGrid(int num_pts, int num_grids, int *grid_rows, int *grid_cols, int num_cameras,
...
#define MIN_POINTS 10
    ...
    do {
        ...
        int num_projections = 0;
        for (int i = 0; i < total_no_grid_pts; i++) 
		{
            num_projections += (int) pt_views[i].size();
        }
		
        vmask = new char[total_no_grid_pts * num_cameras];
        projections = new double[2 * num_projections];
		
        for (int i = 0; i < total_no_grid_pts * num_cameras; i++)
            vmask[i] = 0;
			...
			for(int i=0; i<grid_rows[g]; i++)
			{
				for(int j=0; j<grid_cols[g]; j++)
				{
					...
					for (int k = 0; k < num_views; k++)
					{
						...
						vmask[(nz_count + prevPts + i*grid_cols[g]+j) * num_cameras + c] = 1;
						printf("vmask[%d]:%d\n", (nz_count + prevPts + i*grid_cols[g]+j) * num_cameras + c, 1);
						projections[2 * arr_idx + 0] = GetKey(v,m).m_x;
						projections[2 * arr_idx + 1] = GetKey(v,m).m_y;
        ...
        //run_sfm_grid_opt(nz_count, num_grids, grid_rows, grid_cols, num_cameras, 0, vmask, projections,
		//				 fixed_focal ? 0 : 1, 0,
		//				 m_estimate_distortion ? 1 : 0, 1,
		//				 init_camera_params,
		//				 grid_hor_trans_tmp,
		...
		//				 0, 0,
		//				 NULL, 0,
		//				 0, m_optimize_for_fisheye, eps2, 1);
	...
	for (int i = 0; i < num_cameras; i++) 
	{
			 ...
			 iter++) 
		{
			...
			if (key.m_extra >= 0) {
				...
				b[0] = Vx(init_pts[pt_idx]);
				b[1] = Vy(init_pts[pt_idx]);
				b[2] = Vz(init_pts[pt_idx]);
                                       ...
                                       bool use_init_focal_only)
{
    ...
    for (int i = 0; i < num_images; i++) {
        ...
        for (int j = i+1; j < num_images; j++) {
            ...
            printf("images %d %d, numMatches:%d\n", i, j, num_matches);
...
int BundlerApp::SetupInitialCameraPairWithGrid(int i_best, int j_best,
											   double &init_focal_length_0,
											   double &init_focal_length_1,
											   camera_params_t *cameras,
											   v3_t *points, v3_t *colors,
											   std::vector<ImageKeyVector> &pt_views, 
											   int no_grids, int *grid_rows, int *grid_cols, vector<vector<vector<int> > > &gridMask)
...
#define INITIAL_DEPTH 3.0 // 1000.0 // 3.0 /* was 3.0, fixme! */
	...
	int totalGridPts = 0;
	vector<vector<vector<v3_t> > > grid_points_3d;
	grid_points_3d.resize(no_grids);
	for(int g=0; g<no_grids; g++)
	{
		totalGridPts += grid_rows[g]*grid_cols[g];
		grid_pt[g] = v3_new(0.0, 0.0, 0.0);
		grid_hor_trans[g] = v3_new(0.0, 0.0, 0.0);
		grid_ver_trans[g] = v3_new(0.0, 0.0, 0.0);
		grid_initialization[g] = false;
		grid_points_3d[g].resize(grid_rows[g]);
		for(int i=0; i<grid_rows[g]; i++)
			grid_points_3d[g][i].resize(grid_cols[g]);
	}
	...
	for(int g=0; g<no_grids; g++)
	{
		for(int r=0; r<grid_rows[g]; r++)
		{
			for(int c=0; c<grid_cols[g]; c++)
			{
				if(gridMask[g][i_best][r*grid_cols[g]+c] != -1 && gridMask[g][j_best][r*grid_cols[g]+c] != -1)
				{
					int key_idx1 = list[count].m_idx1;
					int key_idx2 = list[count].m_idx2;
					...
					{
						double x_pt = (x_proj1 / m_init_focal_length) * INITIAL_DEPTH;
						double y_pt = (y_proj1 / m_init_focal_length) * INITIAL_DEPTH;
						double z_pt = INITIAL_DEPTH + cameras[0].t[2];
						grid_points_3d[g][r][c] = v3_new(x_pt, y_pt, z_pt);
					...
					else 
					{
						...
						//printf("point1:%f %f, point2:%f %f\n", x_proj1+1080.0, -y_proj1+1440.0, x_proj2+1080.0, -y_proj2+1440.0);
						
						bool in_front = true;
						double angle = 0.0;
						v3_t tmpPt = Triangulate(p, q, cameras[0], cameras[1], error, in_front, angle, true);
						grid_points_3d[g][r][c] = tmpPt;
					
						//printf("tmpPt:%f %f %f\n", Vx(tmpPt), Vy(tmpPt), Vz(tmpPt));
					...
					unsigned char red = GetKey(i_best,key_idx1).m_r;
					unsigned char green = GetKey(i_best,key_idx1).m_g;
					unsigned char blue = GetKey(i_best,key_idx1).m_b;
					colors[pt_count] = v3_new((double) red, (double) green, (double) blue);
				...
				{
					grid_points_3d[g][r][c] = v3_new(0.0, 0.0, 0.0);
				
					ImageKeyVector views;
					pt_views.push_back(views);
				
					colors[pt_count] = v3_new((double) 0, (double) 0, (double) 0);
				}
    ...
    for (unsigned int i = count; i < num_matches; i++) 
	{
		...
		if (!solved_for_extrinsics) 
		{
			...
			points[pt_count-totalGridPts] = v3_new(x_pt, y_pt, z_pt);
			ImageKeyVector views;
			views.push_back(ImageKey(0, key_idx1));
			views.push_back(ImageKey(1, key_idx2));
			pt_views.push_back(views);
		...
		else 
		{
			...
			v3_t tmpPt = Triangulate(p, q, cameras[0], cameras[1], 
			...
			points[pt_count-totalGridPts] = tmpPt;
	...
	for(int g=0; g<no_grids; g++)
	{
		bool hor_found = false;
		for(int i=0; i<grid_rows[g]; i++)
		{
			int pt1 = -1; int pt2 = -1;
			for(int j=0; j<grid_cols[g]; j++)
			{
				if(Vx(grid_points_3d[g][i][j]) != 0.0 && Vy(grid_points_3d[g][i][j]) != 0.0 && Vz(grid_points_3d[g][i][j]) != 0.0)
				{
					if(pt1 == -1)
						pt1 = j;
					else if(pt2 == -1)
					{
						...
						//find horizontal transformation
						grid_hor_trans[g] = v3_new((Vx(grid_points_3d[g][i][pt2])-Vx(grid_points_3d[g][i][pt1]))/double(pt2-pt1),
												(Vy(grid_points_3d[g][i][pt2])-Vy(grid_points_3d[g][i][pt1]))/double(pt2-pt1),
												(Vz(grid_points_3d[g][i][pt2])-Vz(grid_points_3d[g][i][pt1]))/double(pt2-pt1));
						hor_found = true;
						break;
					}
				}
		...
		for(int i=0; i<grid_cols[g]; i++)
		{
			int pt1 = -1; int pt2 = -1;
			for(int j=0; j<grid_rows[g]; j++)
			{
				if(Vx(grid_points_3d[g][j][i]) != 0.0 && Vy(grid_points_3d[g][j][i]) != 0.0 && Vz(grid_points_3d[g][j][i]) != 0.0)
				{
					if(pt1 == -1)
						pt1 = j;
					else if(pt2 == -1)
					{
						...
						printf("vertical between row:%d and %d at column %d\n", pt1, pt2, i);
						//find vertical transformation
						grid_ver_trans[g] = v3_new((Vx(grid_points_3d[g][pt2][i])-Vx(grid_points_3d[g][pt1][i]))/double(pt2-pt1),
												(Vy(grid_points_3d[g][pt2][i])-Vy(grid_points_3d[g][pt1][i]))/double(pt2-pt1),
												(Vz(grid_points_3d[g][pt2][i])-Vz(grid_points_3d[g][pt1][i]))/double(pt2-pt1));
						ver_found = true;
						break;
					}
				}
		...
		for(int i=0; i<grid_cols[g]; i++)
		{
			for(int j=0; j<grid_rows[g]; j++)
			{
				if(Vx(grid_points_3d[g][j][i]) != 0.0 && Vy(grid_points_3d[g][j][i]) != 0.0 && Vz(grid_points_3d[g][j][i]) != 0.0)
				{
					grid_pt[g] = v3_new(Vx(grid_points_3d[g][j][i])-j*Vx(grid_ver_trans[g])-i*Vx(grid_hor_trans[g]), 
									 Vy(grid_points_3d[g][j][i])-j*Vy(grid_ver_trans[g])-i*Vy(grid_hor_trans[g]), 
									 Vz(grid_points_3d[g][j][i])-j*Vz(grid_ver_trans[g])-i*Vz(grid_hor_trans[g]));
					pt_found = true;
					break;
				}
			}
		...
		if(pt_found && ((grid_cols[g]>1 && hor_found) || grid_cols[g]<1) && ((grid_rows[g]>1 && ver_found) || grid_rows[g]<1))
			grid_initialization[g] = true;
	}
...
#define INITIAL_DEPTH 3.0 // 1000.0 // 3.0 /* was 3.0, fixme! */
    ...
    for (unsigned int i = 0; i < num_matches; i++) {
        ...
        if (!solved_for_extrinsics) {
			...
			v3_t tmpPt = Triangulate(p, q, cameras[0], cameras[1], 
											   error, in_front, angle, true);
			points[pt_count] = tmpPt;
				
            printf(" tri.error[%d] = %0.3f\n", i, error);

            if (error > /*4.0*/ m_projection_estimation_threshold && key_idx1 >= 35) {
                printf(" skipping point\n");
                continue;
            }
        }
...
void BundlerApp::BundleAdjust(int noGrids, int *no_grid_rows, int *no_grid_cols, vector<vector<vector<int> > > &gridMask) 
{
    clock_t start = clock();
    ...
    if (num_init_cams == 0) {
		...
		DumpPointsToPly(m_output_directory, "points000.ply", 
		...
		error0 = RunSFM(curr_num_pts, 2, 0, false,
							...
							NULL, NULL, NULL, NULL, !m_fix_necker);
        delete [] S;
	...
	{
		totalGridPts += no_grid_rows[g]*no_grid_cols[g];
	}
	
	grid_pt = new v3_t[noGrids];
	grid_hor_trans = new v3_t[noGrids];
	grid_ver_trans = new v3_t[noGrids];
	 
	vector<vector<vector<v3_t> > > grid_points_3d;
	grid_points_3d.resize(noGrids);
	for(int g=0; g<noGrids; g++)
	{
		grid_pt[g] = v3_new(0.0, 0.0, 0.0);
		grid_hor_trans[g] = v3_new(0.0, 0.0, 0.0);
	...
	vector<ImageKeyVector> gridViews;
	gridViews.resize(totalGridPts);
	v3_t* gridColors = new v3_t[totalGridPts];
	vector<vector<vector<v3_t> > > gridPts;
	gridPts.resize(noGrids);
	
	vector<int> tmp_added_inv;
	tmp_added_inv.resize(num_images, -1);
	
	for(int i=0; i<curr_num_cameras; i++)
		tmp_added_inv[added_order[i]] = i;
	
	int prevPts = 0;
	for(int g=0; g<noGrids; g++)
	{
		gridPts[g].resize(no_grid_rows[g]);
		for(int r=0; r<no_grid_rows[g]; r++)
		{
			gridPts[g][r].resize(no_grid_cols[g]);
			for(int c=0; c<no_grid_cols[g]; c++)
			{
				gridPts[g][r][c] = v3_new(0.0, 0.0, 0.0);
				colors[prevPts + r*no_grid_cols[g]+c] = v3_new(0.0, 0.0, 0.0);
				for(int img=0; img<curr_num_cameras; img++)
				{
					if(gridMask[g][added_order[img]][r*no_grid_cols[g]+c] != -1)
					{
						printf("Grid pt:%d in img %d:%f %f\n", prevPts + r*no_grid_cols[g]+c, added_order[img], GetKey(added_order[img], prevPts + r*no_grid_cols[g]+c).m_x,  GetKey(added_order[img], prevPts + r*no_grid_cols[g]+c).m_y);
						gridViews[prevPts + r*no_grid_cols[g]+c].push_back(ImageKey(img, prevPts + r*no_grid_cols[g]+c));
					}
				}
			}
		}
		prevPts += no_grid_rows[g]*no_grid_cols[g];
	}
	...
	for(int g=0; g<noGrids; g++)
	{
		for(int r=0; r<no_grid_rows[g]; r++)
		{
			for(int c=0; c<no_grid_cols[g]; c++)
			{
				if(gridViews[prevPts + r*no_grid_cols[g]+c].size() > 1)
				{
					if(Vx(gridPts[g][r][c]) == 0.0 && Vy(gridPts[g][r][c]) == 0.0 && Vz(gridPts[g][r][c]) == 0.0)
					{
						int i_best = added_order[gridViews[prevPts + r*no_grid_cols[g]+c][0].first];
						int j_best = added_order[gridViews[prevPts + r*no_grid_cols[g]+c][1].first];
						
						double x_proj1 = GetKey(i_best,prevPts + r*no_grid_cols[g]+c).m_x;
						double y_proj1 = GetKey(i_best,prevPts + r*no_grid_cols[g]+c).m_y;
						double x_proj2 = GetKey(j_best,prevPts + r*no_grid_cols[g]+c).m_x;
						double y_proj2 = GetKey(j_best,prevPts + r*no_grid_cols[g]+c).m_y;
						v2_t p = v2_new(x_proj1, y_proj1);
						v2_t q = v2_new(x_proj2, y_proj2);
						printf("grid pt at row %d and col %d vis in img%d:%f, %f(%d), vis in img%d:%f %f(%d)\n", r, c, i_best, x_proj1, y_proj1, prevPts + r*no_grid_cols[g]+c,
							   j_best, x_proj2, y_proj2, prevPts + r*no_grid_cols[g]+c);
						bool in_front = true;
						double angle = 0.0;
						double error;
						gridPts[g][r][c] = Triangulate(p, q, cameras[gridViews[prevPts + r*no_grid_cols[g]+c][0].first], cameras[gridViews[prevPts + r*no_grid_cols[g]+c][1].first], error, in_front, angle, true);
						printf("grid pt at row %d and col%d:%f %f %f\n", r, c, Vx(gridPts[g][r][c]), Vy(gridPts[g][r][c]), Vz(gridPts[g][r][c]));
					}
				}
			}
		}
	...
	{
		bool hor_found = false;
		for(int i=0; i<no_grid_rows[g]; i++)
		{
			int pt1 = -1; int pt2 = -1;
			for(int j=0; j<no_grid_cols[g]; j++)
			{
				if(Vx(gridPts[g][i][j]) != 0.0 && Vy(gridPts[g][i][j]) != 0.0 && Vz(gridPts[g][i][j]) != 0.0)
				{
					if(pt1 == -1)
						pt1 = j;
					else if(pt2 == -1)
					{
						...
						//find horizontal transformation
						grid_hor_trans[g] = v3_new((Vx(gridPts[g][i][pt2])-Vx(gridPts[g][i][pt1]))/double(pt2-pt1),
												   (Vy(gridPts[g][i][pt2])-Vy(gridPts[g][i][pt1]))/double(pt2-pt1),
												   (Vz(gridPts[g][i][pt2])-Vz(gridPts[g][i][pt1]))/double(pt2-pt1));
						hor_found = true;
						break;
					}
				}
		...
		bool ver_found = false;
		for(int i=0; i<no_grid_cols[g]; i++)
		{
			int pt1 = -1; int pt2 = -1;
			for(int j=0; j<no_grid_rows[g]; j++)
			{
				if(Vx(gridPts[g][j][i]) != 0.0 && Vy(gridPts[g][j][i]) != 0.0 && Vz(gridPts[g][j][i]) != 0.0)
				{
					if(pt1 == -1)
						pt1 = j;
					else if(pt2 == -1)
					{
						...
						printf("vertical between row:%d and %d at column %d\n", pt1, pt2, i);
						//find vertical transformation
						grid_ver_trans[g] = v3_new((Vx(gridPts[g][pt2][i])-Vx(gridPts[g][pt1][i]))/double(pt2-pt1),
												   (Vy(gridPts[g][pt2][i])-Vy(gridPts[g][pt1][i]))/double(pt2-pt1),
												   (Vz(gridPts[g][pt2][i])-Vz(gridPts[g][pt1][i]))/double(pt2-pt1));
						ver_found = true;
						break;
					}
				}
		...
		for(int i=0; i<no_grid_cols[g]; i++)
		{
			for(int j=0; j<no_grid_rows[g]; j++)
			{
				if(Vx(gridPts[g][j][i]) != 0.0 && Vy(gridPts[g][j][i]) != 0.0 && Vz(gridPts[g][j][i]) != 0.0)
				{
					grid_pt[g] = v3_new(Vx(gridPts[g][j][i])-j*Vx(grid_ver_trans[g])-i*Vx(grid_hor_trans[g]), 
										Vy(gridPts[g][j][i])-j*Vy(grid_ver_trans[g])-i*Vy(grid_hor_trans[g]), 
										Vz(gridPts[g][j][i])-j*Vz(grid_ver_trans[g])-i*Vz(grid_hor_trans[g]));
					pt_found = true;
					break;
				}
			}
	...
	OptimizeForGrid(totalGridPts, noGrids, no_grid_rows, no_grid_cols, curr_num_cameras,
					cameras, added_order, gridColors,
					gridViews);
	*/
...
/* Compute pose of all cameras */
void BundlerApp::BundleAdjustWithGrid(int noGrids, int *no_grid_rows, int *no_grid_cols, vector<vector<vector<int> > > &gridMask) 
{
    clock_t start = clock();
	
    /* Compute initial image information */
    ComputeGeometricConstraints(false, 0, noGrids, no_grid_rows, no_grid_cols);
	...
	grid_hor_trans = new v3_t[noGrids];
	grid_ver_trans = new v3_t[noGrids];
	grid_initialization = new bool[noGrids];
	
    int max_pts = (int) m_track_data.size() + totalGridPts;
    v3_t *points = new v3_t[max_pts];
    v3_t *colors = new v3_t[max_pts];
    std::vector<ImageKeyVector> pt_views;
    ...
    if (num_init_cams == 0) 
	{
        ...
        /* **** Set up the initial cameras **** */
        double init_focal_length_0 = 0.0, init_focal_length_1 = 0.0;
        pt_count = curr_num_pts = SetupInitialCameraPairWithGrid(i_best, j_best, init_focal_length_0, init_focal_length_1,
																 cameras, points, colors, pt_views, 
																 noGrids, no_grid_rows, no_grid_cols, gridMask) ;
		
		int noOtherPoints = curr_num_pts - totalGridPts;
		int prevPoints = noOtherPoints;
		for(int g=0; g<noGrids; g++)
		{
			for(int i=0; i<no_grid_rows[g]; i++ )
			{
				for(int j=0; j<no_grid_cols[g]; j++)
				{
					points[i*no_grid_cols[g]+j+prevPoints] = v3_new(Vx(grid_pt[g])+Vx(grid_hor_trans[g])*j+Vx(grid_ver_trans[g])*i, 
																 Vy(grid_pt[g])+Vy(grid_hor_trans[g])*j+Vy(grid_ver_trans[g])*i, 
																 Vz(grid_pt[g])+Vz(grid_hor_trans[g])*j+Vz(grid_ver_trans[g])*i);
				}
			}
			prevPoints += no_grid_rows[g]*no_grid_cols[g];
		}
        ...
        /* Run sfm for the first time */
        double error0;
		error0 = RunSFMGRID(curr_num_pts, noGrids, no_grid_rows, no_grid_cols, 2, 0, false,
							cameras, points, added_order, colors, pt_views, 0.0, !m_fix_necker);
		
		//for(int i=0; i<curr_num_pts; i++)
		//{
		//	printf("pt%d:%f %f %f\n", i, Vx(points[i]), Vy(points[i]), Vz(points[i]));
		//}
		
		
        printf("  focal lengths: %0.3f, %0.3f\n", cameras[0].f, cameras[1].f);
		
        prevPoints = noOtherPoints;
		for(int g=0; g<noGrids; g++)
		{
			for(int i=0; i<no_grid_rows[g]; i++ )
			{
		 ...
		 round++, curr_num_cameras++) 
	{
		...
		BundleInitializeImage(m_image_data[next_idx], 
							  ...
							  cameras, pt_views);
#else
		BundleInitializeImageFullBundleGrid(next_idx, parent_idx, 
											round, curr_num_pts, noGrids, no_grid_rows, no_grid_cols, gridMask,
											added_order, cameras, points, colors, pt_views);
#endif
		...
		if (!m_skip_add_points) {
			pt_count = 
			BundleAdjustAddAllNewPointsWithGrid(curr_num_pts, curr_num_cameras + 1,
												added_order, cameras, 
												points, colors,
												noGrids, no_grid_rows, no_grid_cols, gridMask,
												dist0, pt_views);
		...
		/* Run sfm again to update parameters */
		RunSFMGRID(curr_num_pts, noGrids, no_grid_rows, no_grid_cols, round + 1, 0, false,
			   cameras, points, added_order, colors, pt_views);
		
		int prevPoints = curr_num_pts-totalGridPts;
		for(int g=0; g<noGrids; g++)
		{
			for(int i=0; i<no_grid_rows[g]; i++ )
			{
		...
		/* Remove bad points and cameras */
		RemoveBadPointsAndCamerasWithGrid(curr_num_pts, totalGridPts, curr_num_cameras + 1, 
										  added_order, cameras, points, colors, pt_views);
    ...
    for (int i = 0; i < curr_num_pts; i++) {
		...
		if(i<noOtherPoints)
		{
			...
			for (int j = 0; j < (int) pt_views[i+totalGridPts].size(); j++) {
			
				int v = pt_views[i+totalGridPts][j].first;
				int vnew = added_order[v];
				pdata.m_views.push_back(ImageKey(vnew, pt_views[i+totalGridPts][j].second));        }
		...
		else
		{
			...
			printf("Point %d is visible in:\n", i-noOtherPoints);
			for (int j = 0; j < (int) pt_views[i-noOtherPoints].size(); j++) {
				
				int v = pt_views[i-noOtherPoints][j].first;
				int vnew = added_order[v];
				pdata.m_views.push_back(ImageKey(vnew, pt_views[i-noOtherPoints][j].second));    
				printf("%d, ", vnew);
			}
	...
	/*for(int i=0; i<grid_rows; i++ )
	{
		...
		{
			PointData pdata;
			pdata.m_pos[0] = Vx(grid_pt)+Vx(grid_hor_trans)*j+Vx(grid_ver_trans)*i;
			pdata.m_pos[1] = Vy(grid_pt)+Vy(grid_hor_trans)*j+Vy(grid_ver_trans)*i;
			pdata.m_pos[2] = Vz(grid_pt)+Vz(grid_hor_trans)*j+Vz(grid_ver_trans)*i;
	...
	//grid_pt[0] = v3_new(-3.09831, 2.00331, -9.32345);
	//grid_hor_trans[0] = v3_new(0.995836, -0.0163026, -0.249072);
	//grid_ver_trans[0] = v3_new(0.0791669, -1.23097, 0.399506);
	
	//OptimizeForGrid(totalGridPts, noGrids, no_grid_rows, no_grid_cols, num_images,
	//				cameras, added_order, gridColors,
	//				gridViews);
...
bool FindAndVerifyCamera(int num_points, int num_grid_pts_solve, v3_t *points_solve, v2_t *projs_solve,
                         int *idxs_solve,
                         ...
                         std::vector<int> &outliers)
{
	...
	for(int i=0; i<num_points; i++)
	{
		...
		printf("point:%f %f %f, proj:%f %f %f\n", Vx(points_solve[i]), Vy(points_solve[i]), Vz(points_solve[i]), Vx(projs_solve[i]), Vy(projs_solve[i]));
	}
	
    if (num_points >= 9) {
        r = find_projection_3x4_ransac(num_points, num_grid_pts_solve, 
                                  ...
                                  bool refine_cameras_and_points)
{
    ...
    bool success = 
        FindAndVerifyCamera(num_pts_solve, 0, points_solve, projs_solve,
...
void BundlerApp::BundleInitializeImageFullBundleGrid(int image_idx, int parent_idx,
													 int num_cameras, int num_points,
													 int num_grids, int *grid_rows, int *grid_cols,
													 vector<vector<vector<int> > > &grid_mask,
													 ...
													 std::vector<ImageKeyVector> &pt_views) 
{
	...
	v3_t *nongrid_points_solve = new v3_t[num_keys];
    v2_t *nongrid_projs_solve = new v2_t[num_keys];
    int *nongrid_idxs_solve = new int[num_keys];
    int *nongrid_keys_solve = new int[num_keys];
	...
	int numGridPtsToSolve = 0;
	
    printf("[BundleInitializeImageFullBundle]: Connecting existing matches...\n");
	
	int totalGridPoints = 0;
	for(int g=0; g<num_grids; g++)
	{
		totalGridPoints += grid_rows[g]*grid_cols[g];
	}
    ...
    for (int i = 0; i < num_cameras; i++) 
	{
		...
		for(int g=0; g<num_grids; g++)
		{
			for(int r=0; r<grid_rows[g]; r++)
			{
				for(int c=0; c<grid_cols[g]; c++)
				{
					if(grid_mask[g][first][r*grid_cols[g]+c] != -1 && grid_mask[g][second][r*grid_cols[g]+c] != -1)
					{
						int idx1 = list[count].m_idx1;
						int idx2 = list[count].m_idx2;
						...
						else 
						{
							...
							points_solve[num_pts_solve] = v3_new(Vx(grid_pt[g])+Vx(grid_hor_trans[g])*c+Vx(grid_ver_trans[g])*r, 
																 Vy(grid_pt[g])+Vy(grid_hor_trans[g])*c+Vy(grid_ver_trans[g])*r, 
																 Vz(grid_pt[g])+Vz(grid_hor_trans[g])*c+Vz(grid_ver_trans[g])*r);
							
							printf("  Connecting existing point %d => %d [%d] (cam: %d)\n",  this_idx, other_idx, pt_idx, other);
						
							printf("point at row %d col %d:%f %f %f\n", r, c, Vx(points_solve[num_pts_solve]), Vy(points_solve[num_pts_solve]), Vz(points_solve[num_pts_solve]));
							...
							if (num_pts_solve > num_keys) 
							{
								printf("Error! Number of points exceeds the number of keys (%d > %d)!\n", num_pts_solve, num_keys);
							}
						}
					}
		...
		for (unsigned int j = count; j < list.size(); j++) 
		{
            int idx1 = list[j].m_idx1;
            int idx2 = list[j].m_idx2;
            ...
            if (GetKey(other,other_idx).m_extra < 0) {
                ...
                * the camera position */
				...
				nongrid_points_solve[num_nongrid_pts_solve] = points[pt_idx-totalGridPoints];
				//printf("point %d:%f %f %f\n", pt_idx, Vx(points_solve[num_pts_solve]), Vy(points_solve[num_pts_solve]), Vz(points_solve[num_pts_solve]));
				
				nongrid_projs_solve[num_nongrid_pts_solve] = v2_new(GetKey(image_idx,this_idx).m_x, GetKey(image_idx,this_idx).m_y);

                nongrid_idxs_solve[num_nongrid_pts_solve] = pt_idx;
                nongrid_keys_solve[num_nongrid_pts_solve] = this_idx;

                num_nongrid_pts_solve++;

                if (num_nongrid_pts_solve+num_pts_solve > num_keys) {
                    printf("Error! Number of points exceeds "
                        "the number of keys (%d > %d)!\n", 
                        num_nongrid_pts_solve+num_pts_solve, num_keys);
	...
	for(int i=0; i<num_nongrid_pts_solve; i++)
	{
		points_solve[i+num_pts_solve] = nongrid_points_solve[i];
		projs_solve[i+num_pts_solve] = nongrid_projs_solve[i];
		idxs_solve[i+num_pts_solve] = nongrid_idxs_solve[i];
		keys_solve[i+num_pts_solve] = nongrid_keys_solve[i];
	}
    ...
    std::vector<int> inliers, inliers_weak, outliers;
    bool success = 
        FindAndVerifyCamera(num_pts_solve+num_nongrid_pts_solve, num_pts_solve, points_solve, projs_solve,
        idxs_solve, Kinit, Rinit, tinit, 
        m_projection_estimation_threshold, 
        16.0 * m_projection_estimation_threshold, /*4.0*/
        inliers, inliers_weak, outliers);
    ...
    if (!m_skip_full_bundle) {
        /* Run sfm again to update parameters */
		RunSFMGRID(num_points, num_grids, grid_rows,grid_cols, num_cameras+1, 0, false,
				   cameras, points, added_order, colors, pt_views);
    }
}
...
int BundlerApp::RemoveBadPointsAndCamerasWithGrid(int num_points, int num_grid_points, int num_cameras, 
												  int *added_order, 
												  camera_params_t *cameras,
												  v3_t *points, v3_t *colors,
												  std::vector<ImageKeyVector> &pt_views)
{
    int num_pruned = 0;
	int num_nongrid_points = num_points - num_grid_points;
    ...
    for (int i = 0; i < num_points; i++) 
	{
        ...
        if(i<num_nongrid_points)
		{
			num_views = (int) pt_views[i+num_grid_points].size();
			...
			for (int j = 0; j < num_views; j++) 
			{
				int v1 = pt_views[i+num_grid_points][j].first;
				...
				{
					int v2 = pt_views[i+num_grid_points][k].first;
			...
			if (RAD2DEG(max_angle) < 0.5 * m_ray_angle_threshold) {
				...
				for (int j = 0; j < num_views; j++) 
				{
					...
					int v = pt_views[i+num_grid_points][j].first;
					int k = pt_views[i+num_grid_points][j].second;
					GetKey(added_order[v], k).m_extra = -1;
				}
			
				pt_views[i+num_grid_points].clear();
		...
		{
			num_views = (int) pt_views[i-num_nongrid_points].size();
			...
			for (int j = 0; j < num_views; j++) 
			{
				int v1 = pt_views[i-num_nongrid_points][j].first;
				
				printf("visible in image %d:%d, %f %f %f\n", added_order[v1], pt_views[i-num_nongrid_points][j].second, pos[0], pos[1], pos[2]);
				...
				for (int k = j+1; k < num_views; k++) 
				{
					int v2 = pt_views[i-num_nongrid_points][k].first;
			...
			if (RAD2DEG(max_angle) < 0.5 * m_ray_angle_threshold) {
				printf("[RemoveBadPointsAndCamera] "
					   "Removing point %d with angle %0.3f\n",
					   i-num_nongrid_points, RAD2DEG(max_angle));
				
				for (int j = 0; j < num_views; j++) 
				{
					// Set extra flag back to 0
					int v = pt_views[i-num_nongrid_points][j].first;
					int k = pt_views[i-num_nongrid_points][j].second;
					GetKey(added_order[v], k).m_extra = -1;
				}
				
				pt_views[i-num_nongrid_points].clear();
