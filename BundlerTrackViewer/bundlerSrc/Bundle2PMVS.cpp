...
class Bundle2PMVS
{
	static void ReadListFile(char *list_file, std::vector<std::string> &files)
	{
	...
	static void ReadBundleFile(char *bundle_file, 
                    std::vector<camera_params_t> &cameras,
                    std::vector<point_t> &points, 
					std::vector<point_t> &pointIndices, double &bundle_version)
	{
		...
		pointIndices.resize(num_images);
	...
	static void WritePMVS(const char *output_path, char *list_file, char *bundle_file,
               std::vector<std::string> images, 
               std::vector<camera_params_t> &cameras,
			   std::vector<point_t> pointIndices,
			   double *imgPoint)
	{
		...
		for (int i = 0; i < num_cameras; i++) {
			...
			if(imgPoint[2] != 0.0)
			{
				...
				double projPoint[3];
				double homPoint[4];
				homPoint[0] = pointIndices[i].pos[0]; homPoint[1] = pointIndices[i].pos[1]; homPoint[2] = pointIndices[i].pos[2];
				homPoint[3] = 1.0;
				matrix_product(3, 4, 4, 1, P,homPoint, projPoint);
				double oldScale = projPoint[2];
			
				double KT[9];
				double KKT[9];
				double KKTinv[9];
				matrix_transpose(3,3,K,KT);
				matrix_product(3,3,3,3,K,KT,KKT);
				matrix_invert(3, KKT,KKTinv);
			
				double imgPointK[3];
				double newScale;
				matrix_product(1,3,3,3,imgPoint,KKTinv, imgPointK);
				matrix_product(1,3,3,1,imgPointK, imgPoint, &newScale);
				newScale = sqrt(1.0 / newScale);
			
				double newK[9];
				matrix_scale(3,3,K, newScale/oldScale, newK);
				matrix_product(3, 3, 3, 4, newK, Ptmp, P);
				matrix_scale(3, 4, P, -1.0, P);
			}
	...
	static void convertBundle2PMVS(char *list_file, char *bundle_file, char *output_path, double *imgPt1=NULL, double *imgPt2=NULL)
	{
		/* Read the list file */
		FILE *f = fopen(list_file, "r");
		if (f == NULL) {
		...
		double imgPoint[3];
		imgPoint[0] = 0.0; imgPoint[1] = 0.0; imgPoint[2] = 0.0;
		if(imgPt1 != NULL && imgPt2 != NULL)
		{
			imgPoint[0] = imgPt1[0]-imgPt2[0]; imgPoint[1] = imgPt1[1]-imgPt2[1]; imgPoint[2] = imgPt1[2] - imgPt2[2];
		}
		...
		ReadBundleFile(bundle_file, cameras, points, pointIndices, bundle_version);

		/* Write camera geometry in the PMVS file format */
		WritePMVS(output_path, list_file, bundle_file, images, cameras, pointIndices, imgPoint);
