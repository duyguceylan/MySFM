...
public:
    BundlerApp() {
        ...
        m_init_focal_length =  2945.455;//2209.091;//3077.074;//2945.455;//2209.091;//3289.091;//3225.806;//1493.0;//700.0;//1765.52;//1562.304810;//2945.455;//1944.887;//4207.792;//3507.954;//3225.806;
        m_initial_pair[0] = -1;
        m_initial_pair[1] = -1;
    ...
    virtual bool OnInit();
	bool OnInitWithGrid(int no_grids, int *no_grid_rows, int *no_grid_cols, vector<vector<vector<int> > > &gridMask);
    ...
    void ComputeTracks(int new_image_start = 0, int num_grids=0, int *num_grid_rows=NULL, int *num_grid_cols=NULL);

    /* Compute geometric information about image pairs */
    void ComputeGeometricConstraints(bool overwrite = false, 
				     int new_image_start = 0, int num_grids=0, int *num_grid_rows=NULL, int *num_grid_cols=NULL);
	...
	int BundleAdjustAddAllNewPointsWithGrid(int num_points, int num_cameras,
											int *added_order,
											camera_params_t *cameras,
											v3_t *points, v3_t *colors,
	...
	int RemoveBadPointsAndCamerasWithGrid(int num_points, int num_grid_points, int num_cameras, 
										  int *added_order, 
										  camera_params_t *cameras,
										  v3_t *points, v3_t *colors,
										  std::vector<ImageKeyVector> &pt_views);
    ...
    void BundleAdjust(int noGrids, int *no_grid_rows, int *no_grid_cols, vector<vector<vector<int> > > &gridMask);
	void BundleAdjustWithGrid(int noGrids, int *no_grid_rows, int *no_grid_cols, vector<vector<vector<int> > > &gridMask);
	...
	int SetupInitialCameraPairWithGrid(int i_best, int j_best,
									   double &init_focal_length_0,
									   double &init_focal_length_1,
									   camera_params_t *cameras,
									   v3_t *points, v3_t *colors,
     ...
     * optimization) */
    void BundleInitializeImageFullBundleGrid(int image_idx, int parent_idx,
					 int num_cameras,
					 int num_points, 
					 int num_grids, int *grid_rows, int *grid_cols,
					 vector<vector<vector<int> > > &grid_mask,
	...
	double RunSFMGRID(int num_pts, int num_grids, int *grid_rows, int *grid_cols, int num_cameras, int start_camera,
					  bool fix_points, camera_params_t *init_camera_params, v3_t *init_pts, 
					  int *added_order, v3_t *colors,
					  std::vector<ImageKeyVector> &pt_views, double eps2 = 1.0e-12, 
					  bool remove_outliers = true);
	double OptimizeForGrid(int num_pts, int num_grids, int *grid_rows, int *grid_cols, int num_cameras,
