...
class RadialUndistort
{
	...
	static void WriteBundleFile(const char *bundle_file, 
	...
	static void UndistortImage(const std::string &in, 
	...
	static void UndistortImages(const char *output_path,
                     ...
                     const std::vector<camera_params_t> &cameras)
	{
		...
		for (int i = 0; i < num_files; i++) {
			...
			std::string basename = files[i].substr(last_slash+1, last_dot - last_slash - 1);
        
			std::string out = std::string(output_path) + "/" + basename + ".rd.jpg";

			UndistortImage(in, cameras[i], out);
		}
	}

	static void WriteNewFiles(const char *output_path,
                   const std::vector<std::string> &files, 
                   const std::vector<camera_params_t> &cameras,
                   const std::vector<point_t> &points)
	{
	...
	static void RunUndistortImages(char *list_file, char *bundle_file, char *output_path) 
	{
