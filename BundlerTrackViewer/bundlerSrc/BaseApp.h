...
public:
	...
	v3_t *grid_pt, *grid_hor_trans, *grid_ver_trans;
	bool *grid_initialization;
	
	void getGridRefPoint(int gridId, float &x, float &y, float &z) {x = Vx(grid_pt[gridId]); y = Vy(grid_pt[gridId]); z = Vz(grid_pt[gridId]); }
	void getGridHorTrans(int gridId, float &x, float &y, float &z) {x = Vx(grid_hor_trans[gridId]); y = Vy(grid_hor_trans[gridId]); z = Vz(grid_hor_trans[gridId]);}
	void getGridVerTrans(int gridId, float &x, float &y, float &z) {x = Vx(grid_ver_trans[gridId]); y = Vy(grid_ver_trans[gridId]); z = Vz(grid_ver_trans[gridId]);}
