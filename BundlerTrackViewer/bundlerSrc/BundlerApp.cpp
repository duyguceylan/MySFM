...
#define MIN_POINT_VIEWS 3 // 0 // 2
		...
		if (m_run_bundle) {
			printf("[BundlerApp::OnInit] Run bundle...\n");
			
			//if (!m_fast_bundle)
			int noGrids = 0;
			int *no_grid_rows = NULL;
			int *no_grid_cols = NULL;
			vector<vector<vector<int> > > gridMask;
				BundleAdjust(noGrids, no_grid_rows, no_grid_cols, gridMask);
			//else
			//	BundleAdjustFast();
...
bool BundlerApp::OnInitWithGrid(int no_grids, int *no_grid_rows, int *no_grid_cols, vector<vector<vector<int> > > &gridMask)
{
	bool load_file = false;
	char *imageList;
	if (argc >= 1) {
	...
	if (m_run_bundle) {
		...
		BundleAdjustWithGrid(no_grids, no_grid_rows, no_grid_cols, gridMask);
		//BundleAdjust(no_grids, no_grid_rows, no_grid_cols, gridMask);
