...
int BundlerApp::BundleAdjustAddAllNewPointsWithGrid(int num_points, int num_cameras,
													int *added_order,
													camera_params_t *cameras,
													...
													int min_views)
{
	...
	{
		totalGridPoints += grid_cols[g]*grid_rows[g];
	}
	
	vector<v3_t> nonGridPoints;
	for(int i=0; i<num_points-totalGridPoints; i++)
	{
		nonGridPoints.push_back(points[i]);
	}
	
	vector<vector<v3_t> > gridPoints;
	gridPoints.resize(num_grids);
	int prevPoints = num_points - totalGridPoints;
	for(int g=0; g<num_grids; g++)
	{
		for(int i=prevPoints; i<prevPoints+grid_cols[g]*grid_rows[g]; i++)
		{
			gridPoints[g].push_back(points[i]);
		}
		prevPoints += grid_cols[g]*grid_rows[g];
	}
    ...
    for (int i = 0; i < num_tracks; i++) 
	{
		...
		int keyId = new_tracks[i][0].second;
		
		if (num_views < min_views) continue;  /* Not enough views */

		//grid point
		if(keyId < totalGridPoints)
		{
			int gridId;
			int prevPts = 0;
			for(int g=0;g<num_grids;g++)
			{
				if(keyId < prevPts + grid_cols[g]*grid_rows[g])
				{
					gridId = g;
					break;
				}
				prevPts += grid_cols[g]*grid_rows[g];
			}
			...
			colors[keyId] = v3_new((double) r, (double) g, (double) b);
			
			/* Set the point index on the keys */
			printf("Add new grid track:%d\n", keyId);
			for (int j = 0; j < num_views; j++) {
				int camera_idx = new_tracks[i][j].first;
				int image_idx = added_order[camera_idx];
				int key_idx = new_tracks[i][j].second;
				GetKey(image_idx, key_idx).m_extra = keyId;
				pt_views[keyId].push_back(ImageKey(camera_idx,key_idx));
				printf("visible in image:%d\n", image_idx);
			}
			
			m_track_data[track_idx].m_extra = keyId;
			
			double error;
			v3_t pt = TriangulateNViews(new_tracks[i], added_order, cameras, error, true);
			
			if (!isnan(error) || error < max_reprojection_error) 
			{
				gridPoints[gridId][keyId-prevPts] = pt;
	...
	for(int g=0; g<num_grids; g++)
	{
		...
		for(int i=0; i<grid_rows[g]; i++)
		{
			...
			{
				if(Vx(gridPoints[g][i*grid_cols[g]+j]) != 0.0 && Vy(gridPoints[g][i*grid_cols[g]+j]) != 0.0 && Vz(gridPoints[g][i*grid_cols[g]+j]) != 0.0)
				{
					if(pt1 == -1)
						pt1 = j;
					else if(pt2 == -1)
					{
						...
						grid_hor_trans[g] = v3_new((Vx(gridPoints[g][i*grid_cols[g]+pt2])-Vx(gridPoints[g][i*grid_cols[g]+pt1]))/double(pt2-pt1),
												   (Vy(gridPoints[g][i*grid_cols[g]+pt2])-Vy(gridPoints[g][i*grid_cols[g]+pt1]))/double(pt2-pt1),
												   (Vz(gridPoints[g][i*grid_cols[g]+pt2])-Vz(gridPoints[g][i*grid_cols[g]+pt1]))/double(pt2-pt1));
						hor_found = true;
						break;
					}
				}
		...
		for(int i=0; i<grid_cols[g]; i++)
		{
			...
			{
				if(Vx(gridPoints[g][j*grid_cols[g]+i]) != 0.0 && Vy(gridPoints[g][j*grid_cols[g]+i]) != 0.0 && Vz(gridPoints[g][j*grid_cols[g]+i]) != 0.0)
				{
					if(pt1 == -1)
						pt1 = j;
					else if(pt2 == -1)
					{
						...
						grid_ver_trans[g] = v3_new((Vx(gridPoints[g][pt2*grid_cols[g]+i])-Vx(gridPoints[g][pt1*grid_cols[g]+i]))/double(pt2-pt1),
												   (Vy(gridPoints[g][pt2*grid_cols[g]+i])-Vy(gridPoints[g][pt1*grid_cols[g]+i]))/double(pt2-pt1),
												   (Vz(gridPoints[g][pt2*grid_cols[g]+i])-Vz(gridPoints[g][pt1*grid_cols[g]+i]))/double(pt2-pt1));
						ver_found = true;
						break;
					}
				}
		...
		for(int i=0; i<grid_cols[g]; i++)
		{
			for(int j=0; j<grid_rows[g]; j++)
			{
				if(Vx(gridPoints[g][j*grid_cols[g]+i]) != 0.0 && Vy(gridPoints[g][j*grid_cols[g]+i]) != 0.0 && Vz(gridPoints[g][j*grid_cols[g]+i]) != 0.0)
				{
					grid_pt[g] = v3_new(Vx(gridPoints[g][j*grid_cols[g]+i])-j*Vx(grid_ver_trans[g])-i*Vx(grid_hor_trans[g]), 
										Vy(gridPoints[g][j*grid_cols[g]+i])-j*Vy(grid_ver_trans[g])-i*Vy(grid_hor_trans[g]), 
										Vz(gridPoints[g][j*grid_cols[g]+i])-j*Vz(grid_ver_trans[g])-i*Vz(grid_hor_trans[g]));
					pt_found = true;
					break;
				}
			}
	...
	//combine non-grid points with grid points
	for(int i=0; i<nonGridPoints.size(); i++)
	{
		points[i] = nonGridPoints[i];
	}
	for(int g=0; g<num_grids; g++)
	{
		for(int i=0; i<grid_cols[g]*grid_rows[g]; i++)
		{
			points[i+nonGridPoints.size()] = gridPoints[g][i];
		}
	}
...
/* Add new points to the bundle adjustment */
int BundlerApp::BundleAdjustAddAllNewPoints(int num_points, int num_cameras,
                                        int *added_order,
                                        camera_params_t *cameras,
                                        v3_t *points, v3_t *colors,
                                        double reference_baseline,
