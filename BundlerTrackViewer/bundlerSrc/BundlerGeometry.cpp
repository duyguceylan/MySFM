...
void BundlerApp::ComputeGeometricConstraints(bool overwrite, int new_image_start, int no_grids, int *grid_rows, int *grid_cols) 
{
	...
	MakeMatchListsSymmetric();
        ...
        // RemoveAllMatches();
	ComputeTracks(new_image_start, no_grids, grid_rows, grid_cols);
		...
		tmpMatches.resize(num_images);
		for(int i=0; i<num_images; i++)
		{
			tmpMatches[i].resize(num_images-1-i);
			for(int j=i+1; j<num_images; j++)
			{
				SetMatch(i,j);
				SetMatch(j,i);
        ...
        for (int i = 0; i < num_tracks; i++) 
		{
            ...
            for (int j = 0; j < num_views; j++) {
                ...
                for (int k = j+1; k < num_views; k++) {
					...
					{
						KeypointMatch kp;
						kp.m_idx1 = track.m_views[j].second;
						kp.m_idx2 = track.m_views[k].second;
						kp.repeatedMatch = false;
						
						tmpMatches[img1][img2-img1-1].push_back(kp);
					}
					else
					{
						KeypointMatch kp;
						kp.m_idx1 = track.m_views[k].second;
						kp.m_idx2 = track.m_views[j].second;
						kp.repeatedMatch = false;
						
						tmpMatches[img2][img1-img2-1].push_back(kp);
					}
                    //SetMatch(img1, img2);
                    //SetMatch(img2, img1);
                }
		...
		for(int i=0; i<num_images; i++)
		{
			for(int j=i+1; j<num_images; j++)
			{
				...
				m_matches.GetMatchList(idx) = tmpMatches[i][j-i-1];
				
				idx = GetMatchIndex(j, i);
				m_matches.GetMatchList(idx) = tmpMatches[i][j-i-1];
			}
		}
...
#define MIN_INLIERS_EPIPOLAR 12
    if (num_inliers >= MIN_INLIERS_EPIPOLAR) {
                                 ...
                                 double ratio) 
{
    ...
    ANNpointArray pts = annAllocPts(num_points, 128);
    ...
    ANNkd_tree *tree = new ANNkd_tree(pts, num_points, 128, 4);
    clock_t end = clock();
