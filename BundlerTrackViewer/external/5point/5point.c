...
int evaluate_Ematrix_with_inliers(int n, v2_t *r_pts, v2_t *l_pts, double thresh_norm,
								  double *F, int *best_inlier, int *inliers, double *score)
{	
    int num_inliers = 0;
    int i;
    double min_resid = 1.0e20;
                        ...
                        double *R_out, double *t_out)
{
	...
	//printf("E:%f %f %f %f %f %f %f %f %f\n", E_best[0], E_best[1], E_best[2], E_best[3], E_best[4], E_best[5], E_best[6], E_best[7], E_best[8]);
...
int compute_pose_ransac_with_inliers(int n, v2_t *r_pts, v2_t *l_pts, 
									 double *K1, double *K2, 
									 double ransac_threshold, int ransac_rounds, 
									 double *R_out, double *t_out, int *inlier_indices)
{
    v2_t *r_pts_norm, *l_pts_norm;
    int i, round;
    double thresh_norm;
    ...
    if (max_inliers > 0) {
        ...
        inliers = evaluate_Ematrix_with_inliers(n, r_pts, l_pts, // r_pts_norm, l_pts_norm, 
												thresh_norm, F_best, &best_inlier,
												inlier_indices, &score);
...
int compute_pose_ransac_driver(int n, double *r_pts, double *l_pts, 
							   double *K1, double *K2, 
							   double ransac_threshold, int ransac_rounds, 
							   double *R_out, double *t_out, int* inlier_indices)
{
	v2_t *r_pts_new;
	v2_t *l_pts_new;
	r_pts_new = malloc(sizeof(v2_t) * n);
    l_pts_new = malloc(sizeof(v2_t) * n);
	
	int i;
	for(i=0; i<n; i++)
	{
		r_pts_new[i] = v2_new(r_pts[i*2], r_pts[i*2+1]);
        l_pts_new[i] = v2_new(l_pts[i*2], l_pts[i*2+1]);
	}
	
	return compute_pose_ransac_with_inliers(n, r_pts_new, l_pts_new, K1, K2, ransac_threshold, ransac_rounds, R_out, t_out, inlier_indices);
