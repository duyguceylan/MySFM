                                    ...
                                    int essential, double *F) 
{
    ...
    if (num_pts < 8) 
	{
		printf("[estimate_fmatrix_ransac] Could not find 8 good correspondences, F-matrix estimation failed\n");
		return 0;
    }
    ...
    for (i = 0; i < num_trials; i++) 
	{
        ...
        if (isnan(Ftmp[0]) || isnan(Ftmp[1]) || isnan(Ftmp[2]) ||
            ...
            isnan(Ftmp[6]) || isnan(Ftmp[7]) || isnan(Ftmp[8])) 
		{
            printf("[estimate_fmatrix_ransac_matches] nan matrix encountered\n");
            nan = 1;
        }
		...
		{
			//printf("FMatrix at iteration %d:%f %f %f %f %f %f %f %f %f\n", i, Ftmp[0], Ftmp[1], Ftmp[2], Ftmp[3], Ftmp[4], Ftmp[5], Ftmp[6], Ftmp[7], Ftmp[8]);
