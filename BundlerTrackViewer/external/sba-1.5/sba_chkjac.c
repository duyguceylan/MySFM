 ...
 *      it yields hx in R^{nobs}
 * jacf points to a function implementing the jacobian of func, whose consistency with
 *     func is to be tested. Given a p in R^{nvars}, jacf computes into the 
 *	   nvis*(Asz+Bsz) + (totalNVis-nvis)(Asz+ATsz)
 *     matrix jac the jacobian of func at p. Note the jacobian is sparse, consisting of
 *     all A_ij, B_ij, Pt_ij, THor_ij, TVer_ij and that row i of jac corresponds to the gradient of the i-th
 *     component of func, evaluated at p.
 * p is an input array of length nvars containing the point of evaluation.
 * idxij, rcidxs, rcsubs, mcon, cnp, pnp, mnp are as usual. Note that if cnp=0 or
 *     pnp=0 a jacobian corresponding resp. to motion or camera parameters
 *     only is assumed.
 * n is the number of points not on the grid and nvis is the number of measurements not on the grid
 * func_adata, jac_adata point to possible additional data and are passed
 *     uninterpreted to func, jacf respectively.
 * err is an array of length nobs. On output, err contains measures
 *     of correctness of the respective gradients. if there is
...
void sba_motstr_chkjac_grid_x(
							  void (*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata),
							  void (*jacf)(double *p, int n, int nvis, int no_grids, int *grid_rows, int *grid_cols, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata),
							  double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, int n, int nvis, int n_grids, int *n_grid_rows, int *n_grid_cols, 
							  int mcon, int cnp, int pnp, int mnp, void *func_adata, void *jac_adata)
{
	printf("checking derivatives...\n");
	const double factor=100.0, one=1.0, zero=0.0;
	double *fvec, *fjac, *pp, *fvecp, *buf, *err;
	
	int nvars, nobs, m, tn, Asz, Bsz, Tsz, ABsz, ATsz, nnz;
	register int i, j, g, ii, jj;
	double eps, epsf, temp, epsmch, epslog;
	register double *ptr1, *ptr2, *pabt;
	double *pa, *pb, *pt;
	int fvec_sz, pp_sz, fvecp_sz, numerr=0;
	
	nobs=idxij->nnz*mnp;
	tn=idxij->nr; m=idxij->nc;//tn:totalNoPoints
	nvars=m*cnp + n*pnp;
	nvars += 3*pnp+n_grids;
	...
	Asz=mnp*cnp; Bsz=mnp*pnp; Tsz=mnp*pnp; ABsz=Asz+Bsz; ATsz=Asz+Tsz*3;
	fjac=(double *)emalloc((nvis*ABsz+(idxij->nnz-nvis)*ATsz)*sizeof(double));
	...
	/* compute the jacobian at p */
	(*jacf)(p, n, nvis, n_grids, n_grid_rows, n_grid_cols, idxij, rcidxs, rcsubs, fjac, jac_adata);
	...
	for(i=0; i<tn; ++i)
	{
		nnz=sba_crsm_row_elmidxs(idxij, i, rcidxs, rcsubs); /* find nonzero A_ij, B_ij, Pt_ij, THor_ij, TVer_ij j=0...m-1, actual column numbers in rcsubs */
		for(j=0; j<nnz; ++j){
			if(rcsubs[j]<mcon) continue; // A_ij, B_ij, Pt_ij, THor_ij, TVer_ij are zero
			...
			if(cnp){
				if(r<nvis)
					ptr1=fjac + r*ABsz; // set ptr1 to point to A_ij
				else
					ptr1=fjac + nvis*ABsz + (r-nvis)*ATsz; // set ptr1 to point to A_ij
				
				pabt=pa + rcsubs[j]*cnp;
				for(jj=0; jj<cnp; ++jj){
					temp=FABS(pabt[jj]);
					if(temp==zero) temp=one;
			...
			if(pnp){
				if(r>nvis)
					continue;
				ptr1=fjac + r*ABsz + Asz; // set ptr1 to point to B_ij
				pabt=pb + i*pnp;
				for(jj=0; jj<pnp; ++jj){
					temp=FABS(pabt[jj]);
					if(temp==zero) temp=one;
			...
			if(n_grids>0)
			{
				...
				for(jj=0; jj<pnp; ++jj){
					ptr1=fjac + nvis*ABsz + (r-nvis)*ATsz + Asz; // set ptr1 to point to P_ij
					pabt=pt + grid_id*pnp*3;
					temp=FABS(pabt[jj]);
					if(temp==zero) temp=one;
					...
					ptr1=fjac + nvis*ABsz + (r-nvis)*ATsz + Asz + Tsz; // set ptr1 to point to THor_ij
					pabt=pt + grid_id*pnp*3 + pnp;
					temp=FABS(pabt[jj]);
					if(temp==zero) temp=one;
					...
					ptr1=fjac + nvis*ABsz + (r-nvis)*ATsz + Asz + Tsz + Tsz; // set ptr1 to point to TVer_ij
					pabt=pt + grid_id*pnp*3 + pnp + pnp;
					temp=FABS(pabt[jj]);
					if(temp==zero) temp=one;
	...
	for(i=0; i<tn; ++i)
	{
		...
		for(j=0; j<nnz; ++j){
			...
			for(ii=0; ii<mnp; ++ii)
			{
				...
				{
					fprintf(stderr, "SBA: gradient %d (corresponding to element %d of the projection of point %d on camera %d) is (err=%g)\n",
							idxij->val[rcidxs[j]]*mnp+ii, ii, i, rcsubs[j], ptr1[ii]);
...
void sba_str_chkjac_x_grid(
	void (*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata),
	void (*jacf)(double *p, int n, int nvis, int no_grids, int *grid_rows, int *grid_cols, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata),
	double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, int n, int nvis, int n_grids, int *n_grid_rows, int *n_grid_cols, 
	int pnp, int mnp, void *func_adata, void *jac_adata)
{
	sba_motstr_chkjac_grid_x(func, jacf, p, idxij, rcidxs, rcsubs, n, nvis, n_grids, n_grid_rows, n_grid_cols, 0, 0, pnp, mnp, func_adata, jac_adata);
}
#if 0
