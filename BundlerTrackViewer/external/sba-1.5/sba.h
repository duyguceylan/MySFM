...
/* simple drivers */
extern int	sba_motstr_levmar_grid(
							 const int n,
							 const int noGrids,
							 const int *grid_rows,
							 const int *grid_cols,
...
extern int
sba_motstr_levmar_grid_x(const int n, const int noGrids, const int *grid_rows, const int *grid_cols,
						const int m, const int mcon, char *vmask, double *p,    
						const int cnp,const int pnp,double *x, double *covx, 
						const int mnp, 
						void (*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata),
						void (*fjac)(double *p, int n, int nvis, int n_gris, int *grid_cols, int *grid_rows, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata),
						void *adata, const int itmax, const int verbose, const double opts[SBA_OPTSSZ],
						double info[SBA_INFOSZ], int use_constraints, camera_constraints_t *constraints,  /* Constraints on camera parameters */
						int use_point_constraints, point_constraints_t *point_constraints);
...
extern int sba_symat_invert_full_BK(double *A, int m);
