 ...
 *
 */
static void sba_motstr_grid_Qs(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata)
{
	//fprintf(stderr, "Computing the function...\n");
	register int i, j, k;
	int cnp, pnp, mnp; 
	int n_other, n_grid;
	int *grid_cols, *grid_rows;
	double *pa, *pb, *pt, *paj, *pbi, *pxij;
	int n, m, nnz;
	...
	n_other = wdata->n_other;
	n_grid = wdata->n_grids;
	grid_cols = wdata->grid_cols;
	grid_rows = wdata->grid_rows;
	proj=wdata->proj;
	proj_adata=wdata->adata;
	
	n=idxij->nr; m=idxij->nc;
	pa=p; pb=p+m*cnp;
	pt = p+m*cnp+n_other*pnp;
	...
	for(j=0; j<m; ++j){
		...
		for(i=0; i<nnz; ++i){
			if(rcsubs[i] < n_other)
			{
				//fprintf(stderr, "Project other point %d\n", rcsubs[i]);
				pbi=pb + rcsubs[i]*pnp;
			...
			{
				double *b = malloc(sizeof(double)*pnp);
				int prevPoints = n_other;
				int gridId;
				int r, c;
				for(k=0; k<n_grid; k++)
				{
					prevPoints += grid_cols[k]*grid_rows[k];
					if(rcsubs[i] < prevPoints)
					{
						prevPoints -= grid_cols[k]*grid_rows[k];
						r = (rcsubs[i]-prevPoints)/grid_cols[k];
						c = (rcsubs[i]-prevPoints)%grid_cols[k];
						gridId = k;
						break;
					}
				}
				...
				for(k=0; k<pnp; k++)
				{
					b[k] = pt[gridId*pnp*3 + k] + pt[gridId*pnp*3 + pnp + k]*(double)(c) + pt[gridId*pnp*3 + pnp + pnp + k]*(double)(r);
				}
				pxij=hx + idxij->val[rcidxs[i]]*mnp; // set pxij to point to hx_ij
					
				(*proj)(j, rcsubs[i], paj, b, pxij, proj_adata); // evaluate Q in pxij
				
				//fprintf(stderr, "Project grid point at (%d,%d):%f %f %f has proj:%f %f\n", r, c, b[0], b[1], b[2], *pxij, *(pxij+1));
...
static void sba_str_grid_Qs(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata)
{
	fprintf(stderr, "Computing the function...\n");
	register int i, j, k;
	int pnp, mnp; 
	...
	for(j=0; j<m; ++j){
		...
		for(i=0; i<nnz; ++i){
			...
			else
			{
				...
				//printf("gridParam:%f %f %f, %f %f %f, %f %f %f\n", pt[gridId*pnp*3], pt[gridId*pnp*3 + 1], pt[gridId*pnp*3 + 2], pt[gridId*pnp*3 + 3], pt[gridId*pnp*3 + 4], pt[gridId*pnp*3 + 5],
				//	   pt[gridId*pnp*3 + 6], pt[gridId*pnp*3 + 7], pt[gridId*pnp*3 + 8]);
				for(k=0; k<pnp; k++)
				{
					b[k] = pt[gridId*pnp*3 + k] + pt[gridId*pnp*3 + pnp + k]*(double)(c) + pt[gridId*pnp*3 + pnp + pnp + k]*(double)(r);
				}
				...
				(*proj)(j, rcsubs[i], b, pxij, proj_adata); // evaluate Q in pxij
				
				//fprintf(stderr, "Project grid point at (%d,%d):%f %f %f has proj:%f %f\n", r, c, b[0], b[1], b[2], *pxij, *(pxij+1));
				//printf("hx[%d]:%f\n", idxij->val[rcidxs[i]]*mnp, hx[idxij->val[rcidxs[i]]*mnp]);
				free(b);
			}
...
int sba_motstr_levmar_grid(
					  const int n,   /* number of non-grid points */
					  const int noGrids, /* number of grids */
					  const int *grid_rows, /* number of rows in each grid */
					  const int *grid_cols, /* number of columns in each grid */
					  ...
					  point_constraints_t *point_constraints)
{
	...
	wdata.mnp=mnp;
	wdata.adata=adata;
	wdata.grid_rows = grid_rows;
	wdata.grid_cols = grid_cols;
	wdata.n_other = n;
	wdata.n_grids = noGrids;
	
	retval=sba_motstr_levmar_grid_x(n, noGrids, grid_rows, grid_cols, m, mcon, vmask, p, cnp, pnp, x, covx, mnp, sba_motstr_grid_Qs, NULL, &wdata, itmax, verbose, opts, info, use_constraints, constraints, use_point_constraints, point_constraints);
	...
	if(info){
		...
		int totalNoPoints = n;
		for(i=0; i<noGrids; i++)
			totalNoPoints += grid_rows[i]*grid_cols[i];
		for(i=nvis=0; i<totalNoPoints*m; ++i)
			nvis+=(vmask[i]!=0);
						...
						double info[SBA_INFOSZ])
{
	...
	retval=sba_str_levmar_grid_x(n, noGrids, grid_rows, grid_cols, m, vmask, p, pnp, x, covx, mnp, sba_str_grid_Qs, NULL, &wdata, itmax, verbose, opts, info);
